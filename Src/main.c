...
int get_sw_state(GPIO_TypeDef *GPIOx, uint16_t GPIO_Pin) {
    int stable_count = 0;
...
int main(void) {
    ...
    int blue_sw_prev = get_sw_state(BLUE_SW_GPIO_Port, BLUE_SW_Pin);
    int yellow_sw_prev = get_sw_state(YELLOW_SW_GPIO_Port, YELLOW_SW_Pin);
    int red_sw_prev = get_sw_state(RED_SW_GPIO_Port, RED_SW_Pin);

    while (1) {
        /* USER CODE END WHILE */

        /* USER CODE BEGIN 3 */
        int sw_blue = get_sw_state(BLUE_SW_GPIO_Port, BLUE_SW_Pin);
        int sw_yellow = get_sw_state(YELLOW_SW_GPIO_Port, YELLOW_SW_Pin);
        int sw_red = get_sw_state(RED_SW_GPIO_Port, RED_SW_Pin);
